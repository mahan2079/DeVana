...
@unittest.skipIf(not CMAES_AVAILABLE, "cma not installed")
class TestCMAESWorker(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_cmaes_worker_run(self):
        """Test if CMAESWorker can run and emit progress"""
        worker = CMAESWorker(
            self.main_params, 
            self.targets, self.weights,
            0, 200, 20,
            cma_initial_sigma=0.3,
            cma_max_iter=2,
            cma_tol=1e-6,
            cma_parameter_data=self.dva_bounds
        )
        
        results = []
        def capture_progress(gen):
            results.append(gen)
        ...
        if worker.isRunning():
            worker.terminate()
            worker.wait()
            self.fail("CMAESWorker timed out")

        self.assertTrue(len(results) > 0, "CMAESWorker did not emit any progress")
