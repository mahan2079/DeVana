...
**Objective 1 - FRF (Frequency Response Minimization - Singular Criteria)**:
$$ f_1(\mathbf{x}) = \sum_{m=1}^{5} \sum_{c \in C_m} w_{m,c} \cdot \frac{A_{m,c}(\mathbf{x})}{T_{m,c}} $$

where:
- $ m \in \{1, 2, 3, 4, 5\} $ = mass index (mass_1 through mass_5)
- $ c \in C_m $ = set of criteria for mass $m$:
  - Peak positions: $ \{\text{peak\_position}_1, \text{peak\_position}_2, \ldots\} $
  - Peak values: $ \{\text{peak\_value}_1, \text{peak\_value}_2, \ldots\} $
  - Bandwidths: $ \{\text{bandwidth}_{i,j} \mid i < j\} $ = frequency differences between peaks
  - Slopes: $ \{\text{slope}_{i,j} \mid i < j\} $ = slope magnitudes between peaks
  - Area under curve: $ \text{area\_under\_curve} $ = integrated magnitude over frequency range
  - Maximum slope: $ \text{slope\_max} $ = maximum absolute slope between any pair of peaks
- $ A_{m,c}(\mathbf{x}) $ = actual value from FRF calculation for mass $m$, criterion $c$
- $ T_{m,c} $ = target value for mass $m$, criterion $c$
- $ w_{m,c} $ = weight assigned to mass $m$, criterion $c$ (zero weight means criterion is ignored)
- $ H(\omega, \mathbf{x}) $ = Frequency Response Function computed for $ \omega \in [\omega_{\text{start}}, \omega_{\text{end}}] $ with $ \omega_{\text{points}} $ discrete frequency points
- **Interpretation**: Minimize the weighted sum of normalized deviations from target values across all masses and criteria. The singular response aggregates multiple frequency response characteristics into a single scalar objective.
